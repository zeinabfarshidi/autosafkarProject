...
$(document).on('click', function (e) {
    ...
    var sidebar__setting = $('.sidebar__setting');
    if ($(e.target).is(settings) || settings.has(e.target).length == 1 || $(e.target).is(sidebar__setting) || sidebar__setting.has(e.target).length == 1) {
        $('.sidebar__setting').addClass('left-0');
    } else {
        $('.sidebar__setting').removeClass('left-0');
    }
})

function acces__status(event, id){
    var current_route_name = $('#current_route_name').val();
    $.ajax({
        method: 'POST',
        url: '/admin/acces',
        data: {id:id, route_name:current_route_name},
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function (data){
            var acces = jQuery.parseJSON(data);
            if (acces['status']){
                $('#acces__' + acces['role_id']).text('block');
                $('#acces__' + acces['role_id']).removeClass('btn-primary');
                $('#acces__' + acces['role_id']).addClass('btn-danger');
            }
            else {
                $('#acces__' + acces['role_id']).text('unblock');
                $('#acces__' + acces['role_id']).addClass('btn-primary');
                $('#acces__' + acces['role_id']).removeClass('btn-danger');
            }
        },
...
// function allInfoAccessSettings(event, role_id){
//     $.ajax({
//         method: 'POST',
//         url: '/admin/accessToInfo',
//         data: {role_id:role_id, class_name:$('.class_name').val()},
//         headers: {
//             'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
//         },
//         success: function (data){
//             var accessToInfo = jQuery.parseJSON(data);
//             if (accessToInfo['status']){
//                 $('#accessToInfo__' + accessToInfo['role_id']).text('block');
//                 $('#accessToInfo__' + accessToInfo['role_id']).removeClass('btn-primary');
//                 $('#accessToInfo__' + accessToInfo['role_id']).addClass('btn-danger');
//             }
//             else {
//                 $('#accessToInfo__' + accessToInfo['role_id']).text('unblock');
//                 $('#accessToInfo__' + accessToInfo['role_id']).addClass('btn-primary');
//                 $('#accessToInfo__' + accessToInfo['role_id']).removeClass('btn-danger');
//             }
//         },
//         error: function (a, b, c){
//         }
