    ...
    <div class="main-content" wire:init="loadInformation">
        <div class="row">
            <div class="col-12 bg-white pb-5">
                ...
                <form action="{{ route('posts.update', $post->id) }}" method="post" class="padding-10" enctype="multipart/form-data" role="form">
                    @csrf
                    @method('put')
                    <div class="row w-div-32 mb-3 jsc-space-between">
                        <div class="mb-3">
                            <input type="text" name="title" value="{{ $post->title }}" placeholder="عنوان مقاله" class="form-control">
                            @error('title')
                        ...
                        <div>
                            ...
                            <div>
                                ...
                                @elseif($post->img)
                                    <img src="{{ asset('storage/' . $post->img) }}" width="400" alt="" class="form-control mt-3 mb-3 border-0">
                                @endif
                            </div>
                        </div>
                    </div>
                    <div wire:ignore>
                        <textarea class="form-control" name="text" id="content">{!! $post->text !!}</textarea>
                        @error('text')
                        <p class="text-danger mt-2 font-size-12">{{ $message }}</p>
                        @enderror
                    </div>
