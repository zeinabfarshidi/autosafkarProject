...
<div>
                    ...
                    <div class="card border-0 font-size-14">
                        <div class="card-header mb-4 pb-0">
                            <div class="d-flex jsc-space-between alc-center">
                                ...
                                <div class="">
                                    <div class="font-size-12">تاریخ درخواست:
                                        <strong>{{ verta($request->created_at)->format('Y/m/d') }}</strong>
                                    </div>
                                    @if(auth()->user()->id == $request->user_id && !$request->tender)
                                        <a href="{{ route('tender.create', $request->id) }}"
                            ...
                            <div class="d-flex btn-status">
                                ...
                                @can('requestConfirmation', $request)
                                    <button
                        ...
                        <div class="card-body d-flex w-div-32 jsc-space-between p-0 m-0">
                            @if($view == 'description')
                                <img src="{{ asset('storage/'. $request->img) }}"
                                     class="w-100 border-radius-3 single-page-request-img" alt="">
                                <p class="mt-4">{!! $request->description !!}</p>
                            @elseif($view == 'serviceProviders' && $request->offers->count() > 0)
    ...
    @if($request->cooperation && !$request->cooperation->do)
            ...
            <div class="swalConfirm d-flex jsc-center alc-center" >
                <div class="card bg-white">
                    <div class="card-header d-flex jsc-space-between alc-center pb-3">
                        <h2 class="swalConfirmTitle">آیا درخواست شما توسط {{ $request->cooperation->offer->user->name }} انجام شد؟</h2>
                        ...
                        <form wire:submit.prevent="resultCooperation()">
                            ...
                            @if($do == 'do')
                                <div class="border mt-4 p-4 border-radius-3">
                                    <p class="text-center">خدمات {{ $request->cooperation->offer->user->name }} چطور بود؟</p>
